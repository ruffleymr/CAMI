...
Neutral.Sims <- SimCommunityAssembly(sims = 100, N = 100, local = 0.5,
                                 traitsim = "BM",
                                 comsim = "neutral")
...
hist(as.numeric(Neutral.Sims$params[,8]), breaks=50, main="Sig2")
```

We can also simulate under alternative models of community assembly and trait evolution, and add/change prior ranges.

```{r message=FALSE}
Filtering.Sims <- SimCommunityAssembly(sims = 100, N = c(100,200), local = 0.5,
                                 traitsim = "OU",
                                 comsim = "filtering",
                                 sig2 = c(2.0, 5.0))
